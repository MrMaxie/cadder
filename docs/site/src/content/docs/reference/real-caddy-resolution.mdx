...
## Shim recovery for `caddy run`

When the PATH-facing shim handles `caddy run`, it first tries to attach to the configured Cadder runtime. If no daemon is available, recovery stays inside Cadder's daemon ownership boundary:

1. Start the missing daemon through Cadder's detached background launch contract.
2. Attach to `cadderd` and register the project definition through the Cadder protocol.
3. Print a terminal error that includes the original attach failure, daemon-start result, and concrete retry guidance.

The shim does not run `caddy run` as an unmanaged real Caddy fallback. The resolver described above is used by `cadderd` to find the real Caddy execution target after the daemon accepts managed state.

Read-only inspection commands such as `caddy version`, `caddy adapt`, and `caddy validate` may still delegate to the safely resolved real Caddy binary. If `cadderd` is unavailable, the shim labels stderr so the command output is understood as real-Caddy inspection, not Cadder runtime state.
