...
4. `profiles.<profile>.real_caddy` in the system file.
5. `defaults.real_caddy` in the system file.
6. A native executable named `caddy` on `PATH` that is not Cadder's shim.
...
Every configured value is an absolute path. Cadder canonicalizes the executable and requires a regular native executable. Windows batch scripts and a final reparse point are rejected; Unix executables require an executable mode bit.
...
PATH fallback ignores empty and relative entries. It checks only the platform-native `caddy` filename and skips candidates that are not native executables.
