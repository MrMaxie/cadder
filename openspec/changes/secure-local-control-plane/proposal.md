...
- `CAD-001`: Real Caddy comes only from trusted sources

## Scope

This change hardens the per-user daemon boundary, protocol negotiation, transport framing, connection supervision, discovery, trusted Caddy resolution, owned-process containment, shim command policy, registration leases, and coordinated shutdown. It migrates the daemon, operator client, and shim to one bounded local IPC client and preserves newline-delimited JSON as the wire transport.
...
- The complete trusted configuration schema, listener-collision checks, retention settings, and status/doctor source presentation remain in their owning runtime, observability, and operator changes together with `RUN-008`. This slice accepts only the trusted real-Caddy selector needed by `CAD-001`.
...
- A project-controlled source cannot select real Caddy; unsafe containing directories, shim links, and identity or digest changes observed at a spawn gate are rejected before project traffic is served.
...
The local wire protocol changes incompatibly before 1.0 and therefore advances its major version. Existing local daemon/client combinations fail with typed upgrade guidance instead of falling back. Real-Caddy selection moves out of project and executable directories into trusted per-user or system configuration. The shim no longer starts the daemon implicitly or delegates mutation-oriented Caddy commands. New direct dependencies are limited to mature libraries for bounded codecs, cancellation/task tracking, executable identity, semantic versions, and process-tree ownership.
