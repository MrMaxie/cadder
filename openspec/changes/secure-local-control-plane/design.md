...
Unit tests cover protocol version negotiation, version-aware capability intersection, recursive additive decoding through every retained public response DTO, gated-mutation compatibility, operation gating before payload decode, closed mutation DTOs, malformed-input classification, typed error round trips, exact codec boundaries, queue byte accounting, trusted selector precedence, containing-directory permissions, executable identity, command policy, durable registration identity, lease transitions, cancellation, and atomic publication generations. Checked fixtures cover every registered mutation, its generated closed JSON Schema, the complete operation and capability registry, the exhaustive protocol-error kind-to-code mapping, and every serialized enum or union discriminator. Their generated contract comparison fails the release test until an intentional wire change receives an IPC-010 compatibility review and updates the readable fixture.
