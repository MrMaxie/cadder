...
- [ ] 4.1 `[CAD-001]` Move real-Caddy selection to explicit daemon override, trusted per-user/system selectors, and safe PATH; validate containing-directory ownership/write permissions; remove project, environment, executable-adjacent, and shim selectors (verification: `cargo test -p cadder-daemon trusted_caddy_source`)
